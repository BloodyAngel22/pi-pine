...
interface ChatState {
  ...
  setSessionName(name: string, tabId?: string | null): Promise<void>;
...
export const useChat = create<ChatState>((rawSet, get) => {
  ...
  async commitPlan() {
    ...
    const prompt = `Реализуй план из файла \`${planFilePath}\`. Важно: контекст был очищен — plan file это твой ЕДИНСТВЕННЫЙ источник информации, истории разговора нет. Сначала прочитай plan file целиком. Перед каждой правкой перечитай указанное место в коде: номера строк могли сместиться, ориентируйся на имена функций/символов из плана. Выполняй задачи по порядку, обновляй чекбоксы в plan file с [ ] на [x], измени Status на "executing", а после завершения на "done". Можешь редактировать код и запускать команды. Если план расходится с реальным кодом — спроси через ask_user (один вопрос на вызов).`;
    try {
  ...
  async setSessionName(name, tabId) {
    const targetId = tabId ?? get().activeTabId;
    try {
      await rpc.setSessionName(name, targetId);
      // rpc.setSessionName со scoped sessionId временно двигает
      // backend-active-session на targetId (см. rpc-mode.ts) — но т.к. мы
      // всегда явно передаём sessionId во все команды, это безопасно.
      // refreshState() тянет состояние ИМЕННО активной (UI) вкладки, так
      // что для фоновой (не активной) вкладки достаточно точечно обновить
      // её локальный sessionName.
      if (targetId === get().activeTabId) {
        await get().refreshState();
      } else if (targetId) {
        get().updateTab(targetId, { sessionName: name });
      }
    } catch (e) {
      get().setError((e as Error).message);
    }
