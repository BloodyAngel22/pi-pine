...
<p align="center">
    Минималистичный десктоп-фронтенд для <a href="https://pi.dev/">pi</a> — AI-кодинг-агента от mariozechner.
    Pi Pine не дублирует логику pi: это тонкая обёртка вокруг <code>pi --mode rpc</code>, убирающая шум CLI
    и дающая удобный однооконный диалог с агентом.
</p>

> **Стек**: Tauri 2 · Rust · React 18 · TypeScript · Vite · Tailwind CSS 4 · Zustand  
> **Статус**: alpha / MVP

## Чем отличается от pi CLI и pi-desktop

| | pi CLI | pi-desktop | Pi Pine |
|---|---|---|---|
| Интерфейс | Терминал | Electron/веб | Tauri (нативный, ~6 МБ) |
| Цель | Полный контроль | Feature-rich IDE | Минимальный чат без шума |
| Thinking / Tool UX | Сырой поток | Богатый UI | Компактные сворачиваемые блоки |
| Размер бандла | — | ~100+ МБ | ~6 МБ |
...
Tauri (Rust): rpc.rs · paths.rs · sessions.rs · plans.rs
...
- **`src/store/chat.ts`** — Zustand: события pi → UI-блоки, регенерация/редактирование через fork, баннер ветки.
...
**Форк не работает / «Не нашёл pi-id»**  
→ Переключитесь на сессию и обратно — сопоставление ID восстановится из истории сессии.
